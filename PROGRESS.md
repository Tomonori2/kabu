...
- 🏠 ホーム: 今月の損益（月初0円リセット）・通算損益・含み損益・🎯月の目標ゲージ
- ➕ 追加: 取引の記録（日付変更可・証券コードで会社名自動表示）＋📷スクショ自動入力
- 📋 履歴: 取引一覧（新しい順）＋取引の修正・削除
- 📊 保有: 現在株価・含み損益・📊6か月チャート・👀ウォッチリスト
- 📈 分析: 勝率・累計損益・月別/銘柄別の成績・月別一覧表・自動コメント
- 🤖 AI: 高坂先生（Gemini）の銘柄分析＋💼ポートフォリオ診断＋チャット追い質問
- DB: trades / settings / watchlist の3テーブル（すべてRLSなしで作成済み）
- Secrets: SUPABASE_URL / SUPABASE_KEY / GEMINI_API_KEY 設定済み
