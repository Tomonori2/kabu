...
- ➕ 追加: 取引の記録（日付変更可・証券コードは任意入力）
- 📋 履歴: 取引一覧（新しい順）＋取引の修正・削除
- 📊 保有: 保有株と、証券コード登録銘柄は現在株価・含み損益
- 📈 分析: 勝率・累計損益・月別/銘柄別の成績・自動コメント
- 🤖 AI: 高坂先生のAI株式分析（Gemini）
  - 決算データ＋Google検索でニュースも反映、チャットで追い質問可
  - Secrets に GEMINI_API_KEY 設定済み（AQ.形式のVertexキーにも対応）
  - 503エラーは Google側の混雑。自動リトライ済みなので再実行すればOK
- テーマ: 海の青系、スマホ最適化済み
- DBの code 列: 追加済み（現在株価機能は有効）
